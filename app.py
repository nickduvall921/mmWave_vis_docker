...
Config priority: /data/config.json (persistent file) > Environment Variables
"""
...
# --- Per-session device tracking instead of global current_topic ---
...
def handle_update_parameter(data):
    ...
    mqtt_client.publish(f"{topic}/set", json.dumps({param: value}))
...
def handle_force_sync():
    ...
    mqtt_client.publish(f"{topic}/set", json.dumps(
        {"mmwave_control_commands": {"controlID": "query_areas"}}
    ))
...
def save_config():
    """Save new configuration to persistent file and return status."""
    try:
        ...
        return json.dumps({'ok': True, 'message': 'Config saved. Restart container to apply.'})
    except Exception as e:
